...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def train(self, setting):
        ...
        try:
            ...
            if os.path.exists(checkpoint_path):
                ...
                if 'optimizer_state_dict' in checkpoint:
                    ...
                    print("model_optim loaded!")
                else:
                    print("Warning: Optimizer state not found in checkpoint!")
                 
                 
                if self.args.use_amp and "scaler_state_dict" in checkpoint and checkpoint["scaler_state_dict"] is not None:
                    scaler.load_state_dict(checkpoint["scaler_state_dict"])
                else:
                    print("Warning: Scaler state not found in checkpoint. Starting with a new scaler.")   
                
                if "scheduler_state_dict" in checkpoint:
                    scheduler.load_state_dict(checkpoint["scheduler_state_dict"])
                else:
                    print("Warning: Scheduler state not found in checkpoint. Starting with a new scheduler.")
        ...
        print("loadedd lr = {:.10f}".format(model_optim.param_groups[0]['lr']))
        #adjust_learning_rate(model_optim, start_epoch+1, self.args)
        #print("adjusted lr = {:.10f}".format(model_optim.param_groups[0]['lr']))
        ...
        for epoch in range(start_epoch,self.args.train_epochs):
            ...
            early_stopping(vali_loss, self.model, path,model_optim,epoch,scaler,scheduler)
        ...
        if self.args.use_multi_gpu:
            dist.barrier()
            self.model.module.load_state_dict(torch.load(best_model_path), strict=False)
        else:
    ...
    def test(self, setting, test=0):
        ...
        if test:
            ...
            load_item = torch.load(os.path.join(self.args.checkpoints, setting, best_model_path), map_location=self.device)
            if self.args.use_multi_gpu:
                dist.barrier()
                self.model.module.load_state_dict(load_item['model_state_dict'], strict=False)
            else:
                self.model.load_state_dict(load_item['model_state_dict'], strict=False)
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                if self.args.visualize and i == len(test_loader)-1:
                    ...
                    visual(gt, pd, os.path.join(dir_path, f'{i}_1.png'))
